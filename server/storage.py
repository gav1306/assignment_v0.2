"""Lightweight SQLite-backed history of pipeline runs.

Each row holds the full PipelineOutput for both the baseline and optimized
pipelines under a shared run_id, written incrementally as each pipeline finishes.
"""
...
DEFAULT_DB_PATH = PROJECT_ROOT / "data" / "runs.sqlite"
...
def save_run(
    ...
    The frontend issues both /run/baseline and /run/optimized with the same
    run_id; whichever finishes first inserts the row and the other updates its
    column.
    """
