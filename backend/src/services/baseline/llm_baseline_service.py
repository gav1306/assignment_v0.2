"""FROZEN baseline LLM client preserved for A/B comparison against the optimized version.

This file intentionally preserves the original bugs from the take-home starter:
- max_tokens=240 (starves the reasoning model -> content=None)
- _chat raises RuntimeError on content=None instead of degrading
- Token counting unimplemented (_stats never incremented)

Do NOT fix these here. The optimized version lives in services/llm_service.py.
"""
...
from src.constants.llm_constants import DEFAULT_MODEL, ENV_OPENROUTER_API_KEY, ENV_OPENROUTER_MODEL
from src.models.pipeline_models import AnswerGenerationOutput, SQLGenerationOutput


class BaselineOpenRouterLLMClient:
    """LLM client using the OpenRouter SDK for chat completions (FROZEN BASELINE)."""
...
def build_default_baseline_llm_client() -> BaselineOpenRouterLLMClient:
    api_key = os.getenv(ENV_OPENROUTER_API_KEY, "").strip()
    if not api_key:
        raise RuntimeError(f"{ENV_OPENROUTER_API_KEY} is required.")
    return BaselineOpenRouterLLMClient(api_key=api_key)
